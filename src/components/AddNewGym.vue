<template>
  <v-card class="rounded-t-lg rounded-b-0">
    <v-card-title>
      <v-row no-gutters>
        Add New {{ type === 'EXC' ? 'Exclusive':'Commercial' }} Gym
        <v-spacer/>
    ...
    <v-card-text>
      <v-form ref="form" v-model="valid">
        ...
        <v-text-field v-model="data.password" :rules="[rules.required]" class="mr-2" outlined label="Password" type="password"/>
        ...
        <v-row no-gutters>
          <v-text-field v-model="data.hourly_rate" label="Hourly Charge" v-if="type === 'EXC'" type="number" min="1" outlined></v-text-field>
          <v-text-field v-model="data.daily_charge" label="Daily Charge" v-if="type !== 'EXC'" type="number" min="1" outlined></v-text-field>
          <v-text-field class="mx-2" v-model="data.weekly_charge" label="Weekly Charge" v-if="type !== 'EXC'" type="number" min="1" outlined></v-text-field>
          <v-text-field v-model="data.monthly_charge" label="Monthly Charge" v-if="type !== 'EXC'" type="number" min="1" outlined></v-text-field>
        </v-row>
...
export default {
  ...
  methods:{
    addNewGym(){
      ...
      if(this.valid){
        this.data.birthday = this.data.bd_mod.year + "-" + this.data.bd_mod.day + "-" + this.data.bd_mod.month ;
        ...
        authClient.post('/register',this.data).then(res => {
          if(res.status === 200){
            this.$emit('close');
          } else {
